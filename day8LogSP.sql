...
MariaDB [csd2204s18]> select * from mnu;
ERROR 1146 (42S02): Table 'csd2204s18.mnu' doesn't exist
...
MariaDB [csd2204s18]> SELECT MANUFACTURER,M.NAME,COUNT(*) "NO. OF ITEMS PRODUCED" FROM 
    -> PRODUCTS AS P INNER JOIN P.CODE=M.NAME;
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near '.NAME' at line 2
MariaDB [csd2204s18]> 
MariaDB [csd2204s18]> SELECT MANUFACTURER,M.NAME,COUNT(*) "NO. OF ITEMS PRODUCED" FROM 
    -> PRODUCTS AS P INNER JOIN manu m
    -> ON m.code=p.manufacturer
    -> GROUP BY MANUFACTURER;
+--------------+-------------------+-----------------------+
...
MariaDB [csd2204s18]> select COUNT(*),manufacturer 
    -> from products as p
    -> INNER JOIN manu m
    -> ON m.code=p.manufacturer WHERE m.code<4
    -> GROUP BY MANUFACTURER;
+----------+--------------+
| COUNT(*) | manufacturer |
+----------+--------------+
...
MariaDB [csd2204s18]> select products.code,products.name,manu.name from manu
    -> LEFT JOIN PRODUCTS ON
    -> products.code=manu.code;
+------+-----------------+-------------------+
| code | name            | name              |
+------+-----------------+-------------------+
|    1 | Hard drive      | Sony              |
...
MariaDB [csd2204s18]> select products.code,products.name,manu.name from products
    -> LEFT JOIN manu ON
    -> products.manufacturer=manu.code;
+------+-----------------+-------------------+
| code | name            | name              |
+------+-----------------+-------------------+
...
MariaDB [csd2204s18]> select products.code,products.name,manu.name"Manufacturer name" from products
    -> LEFT JOIN manu ON
    -> products.manufacturer=manu.code;
+------+-----------------+-------------------+
...
MariaDB [csd2204s18]> select products.code,products.name,manu.name"Manufacturer name" from manu
...
MariaDB [csd2204s18]> select products.code,products.name,manu.name"Manufacturer name" from products
    ...
    -> products.code=manu.code
    -> UNION
    -> select products.code,products.name,manu.name"Manufacturer name" from manu
    -> RIGHT JOIN products ON
    -> products.manufacturer=manu.code;
+------+-----------------+-------------------+
| code | name            | Manufacturer name |
